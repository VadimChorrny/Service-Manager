...
namespace Core.Services
{
    public class SubscriptionService : ISubscriptionService
    {
                        ...
                        if (iElement.Subscription == null || jElement.Subscription == null)
                        {
                            ...
                            if (billingCycle != null)
                            {
                                ...
                                if ((iElement.Description.Contains(jElement.Description) || jElement.Description.Contains(iElement.Description)) && IsPriceDifference(15, iElement.Sum, jElement.Sum))
                                {
                                    ...
                                    subscription.BillingCycle = (await _unitOfWork.BillingCycleRepository.Get(bc => bc.Name == billingCycle)).First();
        ...
        {
            return Enumerable.Range(0, maxDayDifference + 1).Contains(Math.Abs((to.Date - from.Date).Days - 93));
        }
        private bool IsWeekDifference(DateTime from, DateTime to, int maxDayDifference)
        {
            return Enumerable.Range(0, maxDayDifference + 1).Contains(Math.Abs((to.Date - from.Date).Days - 7));
        }
        private bool IsHalfYearDifference(DateTime from, DateTime to, int maxDayDifference)
        {
            return Enumerable.Range(0, maxDayDifference + 1).Contains(Math.Abs((to.Date - from.Date).Days - 183));
        }
